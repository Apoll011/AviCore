...
fn on_sub_syntax_handler(
    ...
    let e_type = inputs[0].get_string_value().unwrap().to_string();
    let event = inputs[1].get_string_value().unwrap().to_string();
    let ident = inputs[2].get_string_value().unwrap();
    let func = &inputs[3];
    ...
    let expected_type =
        EventType::from(e_type.as_str()).ok_or(Box::new(EvalAltResult::ErrorCustomSyntax(
    ...
    match event_name {
        Some(event_name) => {
            ...
            if received_event.event_name.eq_ignore_ascii_case(&event) && received_event.e_type != expected_type {
