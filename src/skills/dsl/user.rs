...
pub fn add_functions(module: &mut Module) {
    ...
    module.add(Arc::new("location".into()), location, Dfn::nl(vec![], Object));
    module.add(Arc::new("quiet_hours".into()), quiet_hours, Dfn::nl(vec![], Object));
    module.add(Arc::new("birthday".into()), birthday, Dfn::nl(vec![], F64));
    module.add(Arc::new("voice_profile_id".into()), voice_profile_id, Dfn::nl(vec![], Str));
...
pub fn nickname(_rt: &mut Runtime) -> Result<Variable, String> {
    Ok(PushVariable::push_var(&runtime().rt.block_on(runtime().user.get_nickname())))
}
...
pub fn location(_rt: &mut Runtime) -> Result<Variable, String> {
    Ok(PushVariable::push_var(&runtime().rt.block_on(runtime().user.get_location())))
}

pub fn quiet_hours(_rt: &mut Runtime) -> Result<Variable, String> {
    Ok(PushVariable::push_var(&runtime().rt.block_on(runtime().user.get_quiet_hours())))
}
...
pub fn voice_profile_id(_rt: &mut Runtime) -> Result<Variable, String> {
    Ok(PushVariable::push_var(&runtime().rt.block_on(runtime().user.get_voice_profile_id())))
}
