...
pub fn add_functions(module: &mut Module) {
    module.ns("slots");
    module.add(Arc::new("require".into()), require, Dfn::nl(vec![Any, Str], Void)); //Gets a slot name and if the slot exists dont do nothing if the sloth dont exist stop de execution by exiting or somthing
    module.add(Arc::new("exists".into()), exists, Dfn::nl(vec![Any, Str], Bool)); //Return true or false if a slot exists
    module.add(Arc::new("get".into()), get, Dfn::nl(vec![Any, Str], Any)); //Gets the Value inside of a slot.value.value
    module.add(Arc::new("get_raw".into()), get_raw, Dfn::nl(vec![Any, Str], Any)); //Gets the raw value inside of a slot.raw_value
    module.add(Arc::new("full".into()), full, Dfn::nl(vec![Any, Str], Object)); //Gets the full sloth object
    module.add(Arc::new("assert_equal".into()), assert_equal, Dfn::nl(vec![Any, Str, Any], Bool)); //Returns true or false if the value of the slot is equal to the value passed as the 2 parameter
    module.add(Arc::new("assert_in".into()), assert_in, Dfn::nl(vec![Any, Str, Any], Bool)); 
    module.add(Arc::new("assert_in_dict".into()), assert_in_dict, Dfn::nl(vec![Any, Str, Any], Bool)); //In key
}
...
pub fn exists(rt: &mut Runtime) -> Result<Variable, String> {
    let name: Arc<String> = rt.pop()?;
    let intent: Intent = rt.pop()?;
    Ok(Variable::Bool(intent.slots.iter().any(|s| s.slot_name == *name), None))
}
...
pub fn assert_in(rt: &mut Runtime) -> Result<Variable, String> {
    ...
    if let Some(slot) = slot {
        ...
        match list {
            Variable::Array(arr) => {
                Ok(Variable::Bool(arr.iter().any(|v| v == &slot_val), None))
            }
...
pub fn assert_in_dict(rt: &mut Runtime) -> Result<Variable, String> {
    ...
    if let Some(slot) = slot {
        ...
        if let Variable::Str(s) = slot_val {
            match dict {
                Variable::Object(obj) => {
                    Ok(Variable::Bool(obj.contains_key(&s), None))
                }
